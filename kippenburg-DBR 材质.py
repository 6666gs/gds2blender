...
#   WAVEGUIDE_BEVEL：True = 给波导加圆角+平滑着色，矩形脊→圆润管状，立体感最强。
#     若波导几何很薄导致圆角异常，把它改 False 即可。
WAVEGUIDE_ROUGHNESS = 0.16
WAVEGUIDE_GLOW = 0.0
WAVEGUIDE_BEVEL = True
WAVEGUIDE_BEVEL_FRAC = 0.35  # 圆角宽度占波导厚度的比例（太圆/穿模就调小）
...
def add_grain(
    ...
    cr.elements[0].color = (clamp(r * e_dark), clamp(g * e_dark), clamp(bl * e_dark), 1.0)
    cr.elements[1].position = 0.80
    cr.elements[1].color = (clamp(r * e_light), clamp(g * e_light), clamp(bl * e_light), 1.0)
...
# ── 波导圆角：给矩形脊波导加 Bevel + 平滑着色，把“扁平脊”读成“圆润管状”，立体感最强 ──
#   ★ 用【精确物体名】(与 assign_material 一致)，避免 "Layer 1/0" 子串误伤 "Layer 1/0.001"。
#   非破坏性修改器，use_clamp_overlap 防穿模；幂等(重复跑只更新同名修改器)。
#   若波导几何很薄导致圆角异常，把顶部 WAVEGUIDE_BEVEL 设 False 即可。
def round_waveguide_edges(obj_names, width_frac=0.35, segments=3):
    names = [obj_names] if isinstance(obj_names, str) else list(obj_names)
    done = 0
    for nm in names:
        ...
        if h <= 0:
            continue
        bev = obj.modifiers.get("FIG_Bevel") or obj.modifiers.new("FIG_Bevel", 'BEVEL')
        bev.width = h * width_frac
        bev.segments = segments
        bev.limit_method = 'ANGLE'
        bev.angle_limit = math.radians(30)
        bev.use_clamp_overlap = True
        # 平滑着色：圆角顺滑、平面仍硬朗（4.1+ 无 use_auto_smooth，靠 Bevel 角度限制即可）
    ...
    print(f"  [波导圆角] 已处理 {done}/{len(names)} 个波导物体")
...
def main():
    ...
    if GROUND_MODE == 'STUDIO':
        if BACKGROUND_STYLE == 'FLAT_DARK':
            ...
            g = FLAT_BG_GRAY * 1.3
            add_studio_ground(center, diag, mins.z, color=(g, g, g * 1.05), roughness=0.55)
    ...
    # ★ 波导圆角 + 平滑：把矩形脊读成圆润管状，立体感最强（“纯受光实心”靠它显形）。
    #   只对青色波导(Layer 41/0)和 RSOA 增益区(Layer 1/0)做；若还有其它波导段，把名字加进列表。
    if WAVEGUIDE_BEVEL:
        round_waveguide_edges(["Layer 41/0", "Layer 1/0"], width_frac=WAVEGUIDE_BEVEL_FRAC)
    ...
    print(
        ...
        f"背景={_bg_label} | 波导=纯受光实心{'+圆角' if WAVEGUIDE_BEVEL else ''} | "
