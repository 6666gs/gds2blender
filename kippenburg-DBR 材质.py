...
#   ★ 这是“整体亮度总开关”，每次出图先调它。换成 softbox 弱光 + 深色背景后，
#     目标是“场景整体偏暗、只让发光波导/谐振跳出来”，一般落在 -2.0 ~ -0.5。
EXPOSURE = -1.2

# ── 背景风格：True=深色径向渐变(发光最跳、酷，配合 Glare 辉光)；False=浅灰影棚(金属反射最均匀) ──
#   你要“突出光在波导里谐振” → 用 True；想要论文里那种干净浅底衬托 → 改 False。
...
def add_camera(name, center, diag, elevation_deg=36.0, azimuth_deg=55.0,
               dist_mult=2.0, lens=50.0, use_ortho=False, ortho_scale_mult=1.25,
               use_dof=False, dof_fstop=2.8, set_active=False):
...
def add_grain(
    ...
    scale = (
        cells / _SCENE_REF if _SCENE_REF else cells
    )  # 物体坐标→颗粒密度，随尺度自适应
...
# ── 发光波导材质工厂（青/红通用，靠参数区分）──────────────────────
#   要“像在发光”而非“上了漆”：让 Emission 唱主角、Base Color 压到接近黑——否则
#   发光体同时反射环境光会发灰、辉光发脏。Emission Strength 要给足，使发光像素在
#   EXPOSURE 压暗后线性亮度仍 > 1，才会被 Compositor 的 Glare 起“辉光/光晕”。
def _emissive_waveguide(name, emit_color, strength, base_tint=None):
    mat, b = _new_material(PREFIX + name)
    if base_tint is None:
        base_tint = tuple(c * 0.06 for c in emit_color[:3])
    _set(b, "Base Color", (*base_tint, 1.0))  # 压暗：发光体不该再大量反光
    _set(b, "Metallic", 0.0)
    _set(b, "Roughness", 0.4)
    _set(b, ["Emission Color", "Emission"], (*emit_color, 1.0))
    _set(b, "Emission Strength", strength)
    return mat, b


def mat_waveguide_cyan():  # 外腔 LNOI 波导/光栅点阵：青白自发光（压暗后仍跳出）
    mat, b = _emissive_waveguide(
        "Waveguide_Cyan",
        emit_color=(0.15, 0.90, 1.0),
        strength=10.0,
        base_tint=(0.0, 0.05, 0.06),
    )
    return mat


def mat_rsoa_red():  # InP RSOA 增益区波导：红色自发光，是注入光的“源头”→ 最亮
    mat, b = _emissive_waveguide(
        "RSOA_Red",
        emit_color=(1.0, 0.10, 0.04),
        strength=16.0,
        base_tint=(0.05, 0.0, 0.0),  # 想更亮/更晕就把 strength 调大
    )
    return mat
...
def main():
    ...
    _purge_prefixed()
    setup_render(scene)
    setup_world_dark(scene) if DARK_BACKGROUND else setup_world(scene)
    ...
    add_camera(
        "1_OutputCloseup", center, diag,
        elevation_deg=30.0, azimuth_deg=135.0, dist_mult=1.9, lens=55.0,
        set_active=(ACTIVE_CAMERA == 1),
    )
    # 机位2「波导俯视」：高仰角 + 正前方(长轴横躺) → 越过金电极看清整体波导走向
    #   想要纯版图(无透视)就把 use_ortho 改 True；想更立体就把 elevation_deg 调小到 ~55
    add_camera(
        "2_WaveguideTop", center, diag,
        elevation_deg=70.0, azimuth_deg=90.0, dist_mult=2.2, lens=48.0,
        use_ortho=False, set_active=(ACTIVE_CAMERA == 2),
    )
    if GROUND_MODE == 'STUDIO':
        if DARK_BACKGROUND:
            # 暗色玻璃台面：会淡淡反出发光波导/光束，很酷；roughness 调小→反射更清晰
            add_studio_ground(
    ...
    # ★ 合成器辉光：让发光波导/谐振真正“晕开发光”。Cycles 必需，放在最后。
    setup_compositor_glow(scene)

    print(
        f"FIG setup done. Resolution: {RES_X} x {RES_Y}  | "
        f"背景={'深色径向' if DARK_BACKGROUND else '浅灰影棚'} | 辉光=on"
    )
