...
给结构根物体下的所有子网格添加 Wireframe 修改器，替换实体面、生成镂空的物理线框。线框粗细 `thickness` 默认 `0.5`（版图单位），在脚本中直接修改。

> ⚠️ **运行前提**：必须先用导入器生成模型。脚本默认查找名为 **`GDS_Chip_Root`** 的根物体，而导入器生成的根物体名取自面板里的**「结构名称」（默认 `GDS_Chip`）**。若两者不一致，脚本会提示「未找到 GDS_Chip_Root」——请把脚本第 5 行的 `chip_root_name` 改成你实际的结构名称。
